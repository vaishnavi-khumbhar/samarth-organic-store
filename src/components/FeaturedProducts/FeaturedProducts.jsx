...
// Keeps categories in a sensible fixed order instead of random object-key order
const CATEGORY_ORDER = ["Oils", "Hair Oils", "Jaggery", "Soap", "Honey", "Ghee"];
...
const ProductCard = ({ item, index }) => {
  ...
  return (
    <div className="group relative bg-white rounded-2xl sm:rounded-3xl overflow-hidden border border-[#eee] hover:border-[#7A2418]/30 shadow hover:shadow-2xl transition duration-300">
      ...
      <button
        ...
        className={`absolute top-3 right-3 sm:top-4 sm:right-4 z-20 w-9 h-9 sm:w-11 sm:h-11 rounded-full flex items-center justify-center transition-all ${
          wished ? "bg-red-500 text-white shadow-lg" : "bg-white text-[#7A2418] hover:bg-[#7A2418] hover:text-white"
        }`}
      >
        <Heart size={16} className="sm:w-[18px] sm:h-[18px]" fill={wished ? "currentColor" : "none"} />
      </button>
        ...
        <div className="p-3 sm:p-5 text-center">
          ...
          <div className="flex justify-center items-center gap-1 sm:gap-1.5 mt-1.5 sm:mt-2">
            <div className="flex gap-0.5 sm:gap-1">
              {[...Array(5)].map((_, i) => (
                <svg key={i} viewBox="0 0 20 20" className="w-3 h-3 sm:w-4 sm:h-4" fill={i < 4 ? "#F5B800" : "#ddd"}>
          ...
          <div className="flex items-center justify-center gap-1.5 sm:gap-2 mt-2 sm:mt-3 flex-wrap">
            <p className="text-[#4D9F38] font-bold text-base sm:text-xl">₹{priceNum}</p>
          ...
          {lowStock && (
            <p className="text-[10px] sm:text-xs font-semibold text-[#B23A3A] mt-1">Only {stock} left!</p>
          )}
        </div>
      </Link>
      ...
      <div className="px-3 sm:px-5 pb-3 sm:pb-5">
        {outOfStock ? (
          <button disabled className="w-full py-2.5 sm:py-3 rounded-xl bg-gray-200 text-gray-500 font-semibold flex items-center justify-center gap-2 cursor-not-allowed">
            Out of Stock
          </button>
        ) : added ? (
          <button
...
const FeaturedProducts = ({ products = [] }) => {
  ...
  // Sort category keys by the fixed order above, then anything unexpected goes last
      ...
      <div className="container-width relative">
        ...
        <div className="text-center">
          ...
          <p className="max-w-xl mx-auto mt-4 text-sm sm:text-base text-gray-600">
            Hand-picked, wood pressed and cold extracted oils, jaggery, honey,
            soaps and ghee — made using traditional methods.
          </p>
        </div>
        ...
        {products.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20">
            <h3 className="text-2xl font-bold text-[#7A2418]">No Products Found</h3>
            <p className="mt-3 text-gray-500">Try changing your search or filter options.</p>
          </div>
        ) : (
            ...
            {categoryKeys.map((category) => (
              <div key={category}>
                ...
                <div className="flex items-center gap-4 mb-6 sm:mb-8">
                  <h3 className="text-xl sm:text-2xl md:text-3xl font-bold text-[#7A2418] shrink-0">
                    {category}
                  </h3>
                  <div className="h-[2px] flex-1 bg-gradient-to-r from-[#F0E4CE] to-transparent" />
                  <span className="text-xs sm:text-sm font-semibold text-[#3C8C2E] bg-[#E4F3E1] px-3 py-1 rounded-full shrink-0">
